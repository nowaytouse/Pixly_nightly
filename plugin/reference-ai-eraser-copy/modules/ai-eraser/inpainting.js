...
const { ensureModel } = require('./cache');
const { getCapabilities } = require('./util.js');
const { currentPath } = require('./data');
const cv = require(`${currentPath}/opencv.js`);
...
function processImage(img, canvasId) {
    return new Promise((resolve, reject) => {
        try {
...
function processMark(img, canvasId) {
    return new Promise((resolve, reject) => {
        try {
            const src = cv.imread(img);
...
function configEnv(capabilities) {
    ort.env.wasm.wasmPaths = url.pathToFileURL(`${currentPath}/`).href;
...
const resizeMark = (image, width, height) => {
    return new Promise((resolve, reject) => {
        const canvas = document.createElement('canvas');
        canvas.width = width;
...
let model = null;
module.exports = async function inpaint(imageBase64, maskBase64) {
