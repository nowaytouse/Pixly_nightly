...
/// Eagle 环境检测
fn is_eagle_environment() -> bool {
    // 检测环境变量 EAGLE_PLUGIN=true（由 JS 设置）
    env::var("EAGLE_PLUGIN").is_ok()
}

#[derive(Parser)]
#[command(name = "pixly-eagle-core")]
#[command(about = "Pixly Eagle Plugin Core - Media Conversion Engine", long_about = None)]
#[command(version = VERSION)]
struct Cli {
    /// Development mode: skip Eagle environment check
...
enum Commands {
    /// 🔍 Analyze media file and get AI recommendations
    Analyze {
        ...
        /// 🔥 File format/extension (from Eagle metadata, for files without extension)
        #[arg(long)]
    ...
    Convert {
        ...
        /// 🔥 Input format/extension (from Eagle metadata, for files without extension)
        #[arg(long)]
...
fn main() {
    ...
    // 🔥 Debug log to verify binary version
    eprintln!("Pixly Core v3.0.1 - Debug Build (No Fallback)");

    let cli = Cli::parse();
    
    // 🔥 Debug: Print command info
    eprintln!("🔍 Debug: CLI Parsed. Command: {:?}", cli.command);
    
    // Eagle 环境检测（开发模式可跳过）
    if !cli.dev && !is_eagle_environment() {
        eprintln!("❌ This binary can ONLY run inside Eagle.");
        eprintln!("   For development, use --dev flag:");
        eprintln!("   $ pixly-eagle-core --dev convert input.jpg --format jxl");
        process::exit(1);
    }
    
    if cli.dev {
        println!("🔧 Development mode: Eagle environment check skipped");
    }
...
fn run(cli: Cli) -> Result<()> {
    match cli.command {
        ...
        Commands::Analyze {
            ...
            // Call analyze module
            use pixly_kernel::cli::cli_analyze::{self, AnalyzeOptions};
        ...
        Commands::Convert {
            ...
            // 🔧 Bug Fix: 清理格式字符串中可能存在的引号（Eagle 有时会返回带引号的格式）
            let target_format = if let Some(user_format) = format {
                // Clean quotes from format string
                let clean_format = user_format.trim_matches(|c| c == '"' || c == '\'').to_lowercase();
                ...
                match selector.select_best_format(&input, Some(&clean_format)) {
                    Ok(recommendation) => {
