...
module.exports = class {
    ...
    static preVerify = async (
        ...
        const isSupportFileSize = async (fileSize) => {
            ...
            const minSize = 0;
            const maxSize = MB_50;
            if (fileSize > maxSize || fileSize < minSize) throw 'fileSizeNotSupported';
        };

        const isSupportResolution = async (type, width, height) => {
            let maxResolution = type === 'webp' ? 16383 : 65535;
            if (width >= maxResolution || height >= maxResolution) throw 'resolutionNotSupported';
            if (width <= 0 || height <= 0) throw 'resolutionNotSupported';
        };
    ...
    static postVerify = async (
        ...
        const isEqualResolution = async (
            ...
            if (!(input_width === output_width && input_height === output_height)) {
                ...
                throw 'fileResolutionIsNotEqual';
            }
        };
        ...
        const isHistogramsAbnormality = async (image1, image2) => {
            ...
            if (similarity <= 0.5) {
                ...
                throw 'fileHistogramsAbnormality';
            }
        };

        const isFileSizeAbnormality = async (output_size) => {
            if (output_size <= 0 || output_size > 10000000000) {
                console.error('fileSizeAbnormality');
                throw 'fileSizeAbnormality';
            }
        };
