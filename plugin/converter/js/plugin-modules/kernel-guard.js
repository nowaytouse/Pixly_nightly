...
(function() {
    ...
    function createKernelWarningUI() {
        ...
        warning.innerHTML = `
            ...
            <p style="font-size: 16px; margin-bottom: 25px; opacity: 0.95;">
                PIXLY插件需要<strong>Rust核心</strong>才能运行<br>
                插件所有功能已被禁用，请立即修复
            </p>
     ...
     * 检测Rust CLI内核
     */
    async function detectRustCLI() {
        const log = window.pixlyLog;
        if (log) {
            log.info('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_DETECTING, {}));
        }
        ...
        try {
            ...
            if (!window.rustCLI) {
                throw new Error('window.rustCLI未初始化');
            }
            
            // 检查available状态
            if (!window.rustCLI.available) {
                throw new Error(window.rustCLI.error || 'Rust CLI不可用');
            }
            ...
            status.rust.detected = true;
            status.rust.path = path;
            status.rust.version = version;
            status.rust.error = null;
            
            if (log) {
                log.info('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_DETECTED, { info: JSON.stringify({ version, path }) }));
            }
            return true;
            
        } catch (error) {
            status.rust.detected = false;
            status.rust.error = error.message;
            
            if (log) {
                log.error('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_DETECTION_FAILED, { error: error.message }));
            }
            return false;
        }
    }
    ...
    async function initKernelGuard() {
        const log = window.pixlyLog;
        if (log) {
            log.info('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_INIT, {}));
        }
        
        // 延迟2秒，确保rustCLI完全初始化
        if (log) {
            log.info('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_WAIT_RUST, { seconds: 2 }));
        }
        await new Promise(resolve => setTimeout(resolve, 2000));
        
        if (log) {
            log.debug('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_CHECK_STATUS, { 
                status: JSON.stringify({
                    exists: !!window.rustCLI,
                    available: window.rustCLI?.available,
                    error: window.rustCLI?.error
                })
            }));
        }
        ...
        if (!detected) {
            ...
            if (log) {
                log.info('KERNEL GUARD', formatLog(LOG.KERNEL_GUARD_INITIALIZED, {}));
            }
        ...
        // 定期检测（每30秒）
        setInterval(async () => {
            if (!window.PIXLY_KERNEL_STATUS.rust.detected) {
                await detectRustCLI();
            }
        }, 30000);
    }
