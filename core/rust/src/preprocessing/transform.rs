...
impl ImageTransformer {
    ...
    fn crop_image(&self, image: &DynamicImage, mode: &CropMode) -> Result<DynamicImage> {
        ...
        info!("📐 裁剪图像: {}x{} -> {}x{}", img_width, img_height, width, height);
    ...
    /// 查找图像中的重要区域（用于智能裁剪）
    fn find_interesting_region(&self, image: &DynamicImage, target_width: u32, target_height: u32) -> (u32, u32) {
        ...
        debug!("智能裁剪：找到最佳区域 ({}, {}), 能量值: {:.2}", best_x, best_y, max_energy);
    ...
    fn calculate_region_energy(&self, gray: &image::GrayImage, 
        ...
        for dy in 1..height-1 {
            for dx in 1..width-1 {
                ...
                let center = gray.get_pixel(px, py)[0] as f32;
...
mod tests {
    ...
    fn test_aspect_ratio_crop() {
        ...
        let transformer = TransformBuilder::new()
            .crop(CropMode::AspectRatio(1.0)) // 1:1 比例
            .build();
