...
impl QualityPredictor {
    ...
    pub fn simd_batch_estimate_ssim(
        &self,
        qualities: &[u8],
        has_alpha: bool,
        complexity: f64,
...
mod tests {
    ...
    #[test]
    fn test_go_ssim_estimation() {
        let predictor = QualityPredictor::new();
        
        // 测试Go算法的基础SSIM预测
        let ssim = predictor.estimate_ssim_before_conversion(85, false, 30.0);
        assert!(ssim >= 0.70 && ssim <= 0.99);
        
        // 测试透明通道影响
        let ssim_alpha = predictor.estimate_ssim_before_conversion(85, true, 30.0);
        assert!(ssim_alpha < ssim); // 有透明通道SSIM应该更低
    }
    ...
    fn test_batch_prediction() {
        ...
        let params = vec![
            ConversionParams { quality: 70, target_format: "webp".to_string(), preserve_alpha: false },
            ConversionParams { quality: 85, target_format: "webp".to_string(), preserve_alpha: false },
            ConversionParams { quality: 95, target_format: "webp".to_string(), preserve_alpha: false },
        ];
