...
mod tests {
    ...
    fn test_conversion_support() {
        ...
        assert!(converter.is_conversion_supported("jpeg", "jxl"));
        assert!(converter.is_conversion_supported("png", "avif"));
        assert!(converter.is_conversion_supported("webp", "jxl"));
        assert!(!converter.is_conversion_supported("gif", "unknown"));
    }
    
    #[test]
    fn test_optimal_tool_selection() {
        let converter = FormatConverter::new();
        
        assert_eq!(
            converter.get_optimal_tool("jpeg", "jxl"),
            Some(ConversionTool::CJXL)
        );
        
        assert_eq!(
            converter.get_optimal_tool("png", "avif"),
            Some(ConversionTool::AVIFENC)
        );
    }
    ...
    fn test_conversion_complexity() {
        ...
        let complexity1 = converter.calculate_conversion_complexity("jpeg", "webp", 85, false);
        let complexity2 = converter.calculate_conversion_complexity("png", "avif", 95, true);
        
        assert!(complexity1 > 0.0);
        assert!(complexity2 > complexity1); // AVIF + lossless should be more complex
    }
    ...
    fn test_recommended_settings() {
        ...
        let web_settings = converter.get_recommended_settings("jpeg", "jxl", "web");
        assert!(web_settings.is_some());
        
        let settings = web_settings.unwrap();
        assert!(settings.contains_key("quality"));
        assert!(settings.contains_key("effort"));
    }
}
