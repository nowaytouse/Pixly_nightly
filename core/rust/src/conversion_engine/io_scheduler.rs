...
mod tests {
    ...
    #[tokio::test]
    async fn test_io_scheduler_creation() {
        let scheduler = IOScheduler::new(true);
    ...
    async fn test_file_read_write() {
        let scheduler = IOScheduler::new(false);
        let temp_file = NamedTempFile::new().unwrap();
        let test_data = b"Hello, PIXLY I/O Scheduler!";
        
        // 写入测试
        let write_request = IORequest {
            operation: IOOperation::Write { data: test_data.to_vec() },
        ...
        let write_result = scheduler.execute_io(write_request).await.unwrap();
        assert!(write_result.success);
        assert_eq!(write_result.bytes_processed, test_data.len() as u64);
        
        // 读取测试
        let read_request = IORequest {
        ...
        let read_result = scheduler.execute_io(read_request).await.unwrap();
        assert!(read_result.success);
        assert_eq!(read_result.data.unwrap(), test_data);
    }
}
