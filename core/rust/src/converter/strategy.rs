...
mod tests {
    ...
    impl ConversionStrategy for MockStrategy {
        ...
        fn convert(
            ...
            Ok(ConversionResult {
                success: true,
                output_path: "test.avif".to_string(),
                input_size: 1000,
                output_size: 500,
    ...
    fn test_strategy_manager() {
        ...
        manager.register(Box::new(MockStrategy {
            name: "Native AVIF".to_string(),
            formats: vec!["avif".to_string()],
        }));
    ...
    #[test]
    fn test_supported_formats_deduplication() {
        let mut manager = StrategyManager::new();
        
        // Register multiple strategies supporting same format
        manager.register(Box::new(MockStrategy {
        ...
        manager.register(Box::new(MockStrategy {
            ...
            formats: vec!["avif".to_string(), "png".to_string()],
        }));
        
        let formats = manager.supported_formats();
        
        // Should deduplicate avif
        let avif_count = formats.iter().filter(|f| *f == "avif").count();
        assert_eq!(avif_count, 1);
    ...
    fn test_conversion_result_compression_ratio() {
        ...
        assert_eq!(result.output_size * 4, result.input_size);
    }

    #[test]
    fn test_global_manager_initialization() {
        let manager = init_global_manager();
        assert!(manager.lock().is_ok());
        
        // Should return same instance
        let manager2 = init_global_manager();
