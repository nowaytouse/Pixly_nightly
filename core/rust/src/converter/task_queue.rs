...
mod tests {
    ...
    fn test_task_creation() {
        let task = ConversionTask::new(
            PathBuf::from("/test/input.png"),
            PathBuf::from("/test/output.jxl"),
            "jxl".to_string(),
            "balanced".to_string(),
        );
    ...
    fn test_queue_add_task() {
        ...
        let task_id = queue.add_task(task).unwrap();
    ...
    fn test_queue_priority() {
        ...
        // 添加不同优先级的任务
        let mut task1 = ConversionTask::new(
            PathBuf::from("/test/1.png"),
            PathBuf::from("/test/1.jxl"),
            "jxl".to_string(),
            "balanced".to_string(),
        );
        task1.priority = TaskPriority::Low;
        
        let mut task2 = ConversionTask::new(
            PathBuf::from("/test/2.png"),
            PathBuf::from("/test/2.jxl"),
            "jxl".to_string(),
            "balanced".to_string(),
        );
        ...
        // 应该先弹出高优先级任务
        let next_task = queue.pop_next_task().unwrap();
        assert_eq!(next_task.priority, TaskPriority::Urgent);
    }
}
