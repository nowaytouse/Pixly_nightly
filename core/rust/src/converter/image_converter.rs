...
impl ImageConverter {
    ...
    fn convert_to_webp(&self, input: &Path, output: &Path) -> Result<()> {
        ...
        if !output_result.status.success() {
            bail!("cwebp 失败: {}", String::from_utf8_lossy(&output_result.stderr));
        }
    ...
    fn convert_to_avif(&self, input: &Path, output: &Path) -> Result<()> {
        ...
        let output_result = cmd.output()
            .context("执行 avifenc 失败")?;
        
        if !output_result.status.success() {
            bail!("avifenc 失败: {}", String::from_utf8_lossy(&output_result.stderr));
        }
    ...
    fn convert_to_jxl(&self, input: &Path, output: &Path) -> Result<()> {
        ...
        if !output_result.status.success() {
            bail!("cjxl 失败: {}", String::from_utf8_lossy(&output_result.stderr));
        }
    ...
    fn convert_to_gif(&self, input: &Path, output: &Path) -> Result<()> {
        ...
        if !output_result.status.success() {
            bail!("ffmpeg 失败: {}", String::from_utf8_lossy(&output_result.stderr));
        }
