...
impl EagleAdapter {
    ...
    pub fn cleanup_old_previews<P: AsRef<Path>>(
        ...
        for entry in fs::read_dir(info_path)? {
            ...
            if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                if filename.starts_with("_pixly_viewer_") && filename.ends_with(".png") {
    ...
    pub fn scan_library(&self) -> Result<Vec<PathBuf>> {
        ...
        if !images_dir.exists() {
            bail!("Eagle库images目录不存在: {:?}", images_dir);
        }
        
        // 递归扫描images目录
        self.scan_directory(&images_dir, &mut image_dirs)?;
        
        info!("📂 扫描Eagle库完成: 找到 {} 个图像", image_dirs.len());
        Ok(image_dirs)
    }
    ...
    pub fn find_xmp_resource<P: AsRef<Path>>(
        ...
        println!("   🔍 搜索Eagle库中的同名XMP资源: {}", image_name);
        println!("   🔍 当前.info目录: {:?}", current_path);
        println!("   🔍 images目录: {:?}", images_dir);
        
        // 遍历images/目录下的所有.info目录
        let entries = match std::fs::read_dir(images_dir) {
            Ok(e) => e,
            Err(e) => {
                println!("   ⚠️  无法读取Eagle images目录: {}", e);
                return None;
            }
        };
        
        println!("   🔍 开始遍历images目录中的.info目录");
        ...
        println!("   🔍 遍历完成: 共检查了 {} 个条目", found_count);
        println!("   ℹ️  未找到Eagle库中的同名XMP资源");
        None
    }
    ...
    pub fn batch_optimize_with_concurrency(
        ...
        info!("🚀 开始批量优化 {} 个图像到 {} 格式", total, format.to_uppercase());
        info!("⚙️  并发线程数: {}", num_threads);
        if dry_run {
            info!("⚠️  DRY RUN模式：不会实际转换文件");
        }
            ...
            image_dirs
                ...
                .map(|info_dir| {
                    ...
                    if current.is_multiple_of(10) || current == total {
                        info!("📊 进度: {}/{} ({:.1}%)", current, total, progress);
                    }
        ...
        info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        info!("📁 总文件数: {}", total);
        info!("✅ 成功处理: {}", total_processed);
        info!("⏭️  已跳过: {}", total_skipped);
        info!("❌ 失败: {}", total_failed);
        info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        info!("📦 原始总大小: {:.2} MB", total_original as f64 / 1024.0 / 1024.0);
        info!("📦 优化后大小: {:.2} MB", total_optimized as f64 / 1024.0 / 1024.0);
        info!("💾 节省空间: {:.2} MB", total_saved as f64 / 1024.0 / 1024.0);
        info!("📊 压缩率: {:.2}%", compression_ratio);
        info!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    ...
    fn optimize_single_image(
        ...
        if current_format == format {
            debug!("⏭️  跳过: 已经是 {} 格式", format.to_uppercase());
        ...
        // 3. 执行转换（调用StrategyManager）
        use crate::converter::strategy::{ConversionConfig, StrategyType, get_global_manager};
        ...
        let config = ConversionConfig {
            ...
            prediction_data: None,  // 🔥 Phase 40.21: Eagle批处理暂不使用AI预测
        };
        ...
        // 执行转换
        let manager_lock = get_global_manager()
            .ok_or_else(|| anyhow::anyhow!("Strategy manager not initialized"))?;
        let manager = manager_lock.lock()
            .map_err(|e| anyhow::anyhow!("Failed to lock strategy manager: {}", e))?;
        
        info!("🔄 转换: {:?} → {:?}", 
                  original_file.file_name().unwrap_or_default(),
                  output_file.file_name().unwrap_or_default());
        
        manager.convert(&original_file, &output_file, format, &config)?;
        ...
        if original_xmp.exists() {
            info!("📄 发现XMP sidecar: {:?}", original_xmp.file_name());
            ...
            match merge_result {
                Ok(output) if output.status.success() => {
                    info!("✅ XMP已合并到目标文件: {:?}", output_file.file_name());
                    
                    // 验证合并成功后删除原始XMP
                    if let Err(e) = std::fs::remove_file(&original_xmp) {
                        warn!("⚠️  删除原始XMP失败: {}", e);
                    } else {
                        info!("🗑️  原始XMP sidecar已删除");
                    }
                },
                Ok(output) => {
                    error!("❌ XMP合并失败: {}", String::from_utf8_lossy(&output.stderr));
                    warn!("⚠️  保留原始XMP sidecar: {:?}", original_xmp);
                },
                Err(e) => {
                    error!("❌ 无法执行exiftool: {}", e);
                    warn!("💡 请安装exiftool: brew install exiftool");
                    warn!("⚠️  保留原始XMP sidecar: {:?}", original_xmp);
                }
            }
        }
        ...
        if original_file != output_file {
            if let Err(e) = std::fs::remove_file(&original_file) {
                error!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                error!("❌ [CRITICAL] 删除原始文件失败！");
                error!("📁 File: {:?}", original_file);
                error!("💥 Error: {}", e);
                error!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                error!("⚠️  原始文件可能仍然存在，请手动检查");
            } else {
                info!("🗑️  原始文件已删除: {:?}", original_file.file_name());
            }
        } else {
            info!("ℹ️  输入输出同名，跳过删除原始文件");
        }
        
        info!("✅ 转换成功: {} MB → {} MB, 节省 {:.2} MB", 
