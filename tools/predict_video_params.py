...
class VideoParamsPredictor:
    ...
    def _predict_encoding_params(self, features: Dict, video_type_info: Dict, 
        ...
        video_type = video_type_info['type']
        source_codec = features.get('codec', '').lower()
        
        # 🎯 核心原则：维持质量前提下，不降级编码器
        # 检测源编码器，防止降级
        if source_codec in ['hevc', 'h265']:
            default_encoder = 'libx265'  # HEVC源保持HEVC
        elif source_codec in ['av1']:
            default_encoder = 'av1'  # AV1源保持AV1
        elif source_codec in ['vp9']:
            default_encoder = 'vp9'  # VP9源保持VP9或升级
        else:
            default_encoder = 'libx264'  # H.264或未知源使用H.264
