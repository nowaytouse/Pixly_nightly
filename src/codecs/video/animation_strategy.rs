...
pub enum AnimationStrategy {
 /// FFmpegprocessing（大frame数GIF）
 FFmpegBased,
 /// frame拼接（小frame数animation）
 FrameStitching,
 /// 直接conversion（staticor极少frame）
 DirectConversion,
 /// outputforvideo
 VideoOutput,
}
...
pub structure AnimationInfo {
 ...
 pub height: u32,
 pub file_size: u64,
 pub has_alpha: bool, // whetherhas透明channel - 关键！
}

/// animation保留configuration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub structure AnimationPreservation {
 /// 保留originalframe数（defaulttrue）
 pub preserve_frame_count: bool,
 /// 保留originalFPS（defaulttrue）
 pub preserve_fps: bool,
 /// originalframe数
 pub original_frame_count: u32,
 /// originalFPS
 pub original_fps: f32,
 /// targetFPS（None表示useoriginalFPS）
...
impl Default for AnimationPreservation {
 fn default() -> Self {
 Self {
 preserve_frame_count: true, // default保留帧数
 preserve_fps: true, // default保留FPS
...
pub structure AnimationStrategySelector;

impl AnimationStrategySelector {
 /// selectoptimalstrategy
 /// based onactualfilefeature，而notformatname！
 /// strategyfully由file内容决定：frame数、transparency度、resolution
 pub fn select_strategy(
 info: &AnimationInfo,
 _target_format: &str, // not再using！格式not决定策略！
 output_as_video: bool,
 ) -> AnimationStrategy {
 // if明确requirementoutputforvideo
 if output_as_video {
...
/// animation转video Converter
pub structure AnimationToVideoConverter;
...
mod tests {
 ...
 assert_eq!(fps, 60.0); // 保持originalFPS
 }
