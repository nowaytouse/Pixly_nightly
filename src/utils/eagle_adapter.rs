/**
 * ==========================================
 * Eagle Adapter - Eagleintegrateddevice
 * ==========================================
 *
 * implementationwithEaglesoftdepthintegrated:
 * - autoEaglestructure
 * - generatepreview and 缩略
 * - Eagleelementdata and label
 * - support.infodirectorystructure
 * - 🔥 Phase 40.15: andbatchprocess
 *
 * Eagledirectorystructure:
 * - images/xxx.library/
 * - xxx.png (original)
 * - .info/
 * - xxx.json (elementdata)
 * - xxx_thumbnail.png (缩略)
 * ==========================================
 */
...
use std::fs;
use rayon::prelude::*; // 🔥 Phase 40.15: andprocess
use std::sync::atomic::{AtomicUsize, Ordering}; // 🔥 Phase 40.15: threaddevice
// 🔧 UnifiedloggingSystem
use tracing::{info, warn, error, debug};

/// Eagleimageelementdata
...
/// height (not片resourcemay has thisfield)
 pub height: Option<u32>,
/// width (not片resourcemay has thisfield)
 pub width: Option<u32>,
/// most after modifiedtime
...
///
/// forvalidationpath、librarylevelconfiguration etc
...
///
/// XMPfilethen：
/// - standard：`image.xmp` (originalextension，.xmp)
/// - like：`photo.jpg` → `photo.xmp`
///
/// # parameter
...
// 🔥 fixed：replaceextensionfor.xmp，while not isappend
// error：image.jpg → image.jpg.xmp
// positive：image.jpg → image.xmp
 xmp_path.set_extension("xmp");
 xmp_path
 }

/// validationpathisnoatcurrentresourcelibraryinside
///
/// # parameter
/// - `path`: validationpath
///
/// # return
/// - `true`: pathatlibraryinside
/// - `false`: pathatlibraryoutside
...
///
/// # parameter
/// - `info_dir`: .infodirectorypath (like images/XXXXX.info/)
///
/// # return
...
/// get缩略path
///
/// Eaglegenerate缩略then: {original_name}_thumbnail.png
///
/// : <https://cn.eagle.cool/support/article/why-do-some-images-generate-extra-thumbnails>
///
/// note: partial片 will outside_thumbnail缩略，thisis Eagleautooptimizationmachine
/// forquickpreview and listdisplay，loadlarge
...
/// generateEagle缩略
///
/// forlargeorspecificformatgenerateoptimization缩略
///
/// # parameter
/// - `source`: source片path
/// - `info_dir`: .infodirectorypath
/// - `size`: 缩略dimension (default800px)
...
// calculation缩略dimension (keepwidehigh)
...
// modifiedtimesort (latestat before )
 previews.sort_by(|a, b| b.1.cmp(&a.1));

// deleteexceedskeep_countpreview
 let mut deleted = 0;
...
/// 🔥 Phase 40.11: Eaglelibrary has image
///
/// librarydirectory，find has containsmetadata.jsonsubdirectory
...
/// 🔥 Phase 40.26: find EaglelibraryXMPresource
///
/// Eaglelibrarystructure：everyresource（片/XMP）all has .infodirectory
/// images/
/// ├── XXXX.info/ (片resource)
/// │ ├── photo.jpg
...
///
/// passmetadata.jsonnamefield关联resource
///
/// parameter：
/// - current_info_dir: current片.infodirectory
/// - image_name: 片namefield（ not extension）
///
...
// images/directorydown has .infodirectory
...
// onlyprocessing.infodirectory，skipcurrent片.infodirectory
...
// readandparsemetadata.json
 ...
 Ok(metadata) => {
// 🔥 Phase 40.33: onlyatfindtomatch when outputlogging，reducelogging
...
/// recursiondirectory
 ...
 if path.is_dir() {
// checkisnocontainsmetadata.json（Eagleimagedirectory）
 let metadata_path = path.join("metadata.json");
 if metadata_path.exists() {
 image_dirs.push(path.clone());
 }

// continuerecursionsubdirectory
...
/// - dry_run: isnoonlytest（ not actualconversion）
///
/// batchoptimizationlibrary（sequentialprocessing）
...
///
/// originalthen（@PROJECT_QUALITY_MANIFESTO.md）：
/// - realconcurrentprocessing， not issimulated
/// - threadsecurityprogresstrace
/// - error not will be 
///
/// # parameter
/// - `format`: targetformat
/// - `quality`: qualityparameter
/// - `dry_run`: isnoonlysimulated
/// - `max_threads`: maximumthread（0=autodetection）
...
// threadsecurity计（forreal-timeprogress）
 let processed = AtomicUsize::new(0);
// note: statisticsdata（failed, saved_bytes）atresultsegmentcalculation

// configuration Rayonthreadpool
