// 📊 multi元线性回归 - from Go ML代码extraction
// forprediction: Quality = β0 + β1*File Size + β2*Resolution + β3*Complexity + ...
//
// Corealgorithm (from Go代码extraction):
// - minimumtwo乘法 (Ordinary Least Squares)
// - 岭回归 (Ridge Regression) - processingmulti重共线性
// - R²决定系数calculation
// - Mean squared error (MSE) calculation

use std::collections::HashMap;

/// multi元线性回归model (from Go Multiple Linear Regressionextraction)
pub structure MultipleLinearRegression {
...
impl MultipleLinearRegression {
 /// createnewmulti元线性回归model
 pub fn new() -> Self {
 Self {
 coefficients: Vec::new(),
 feature_names: vec![
 "intercept".to_string(), // β0 截距
 "file_size_mb".to_string(), // β1 file大小
 "megapixels".to_string(), // β2 百万pixel
 "complexity".to_string(), // β3 复杂度
 "has_alpha".to_string(), // β4 透明度
 "is_animated".to_string(), // β5 动画
 "estimated_quality".to_string(), // β6 当beforequality
 ...
 /// trainingmodel - minimumtwo乘法 (from Go代码extraction)
 /// 
 /// # parameter
 /// * `x` - feature矩阵 \[n_samples, n_features\]
 /// * `y` - targetvalue \[n_samples\] (如quality)
 ...
 /// 岭回归 (Ridge Regression) - processingmulti重共线性 (from Go代码extraction)
 /// β = (X^T * X + λI)^-1 * X^T * y
 ...
 /// prediction (from Go代码extraction)
 ...
 /// calculationstatistics指标 (from Go代码extraction)
 ...
 // calculation SST (总平方 and ) and SSE (error平方 and )
 let mut sst = 0.0;
 let mut sse = 0.0;
 for i in 0..n {
...
// --- 矩阵运算helperfunction (from Go代码extraction) ---
...
/// 矩阵求逆 (high斯-若尔当消元法) (from Go代码extraction)
