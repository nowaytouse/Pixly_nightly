...
//! 
//! fullimplementation Python ML Bridge所需128dimensionalfeature向量
//! 
//! featuregroup：
//! - Basic (16dimensional): 基本property
//! - Color (16dimensional): color分布 and statistics
//! - Texture (16dimensional): 纹理 and 边缘information
//! - Shape (16dimensional): 几何 and structure
//! - Quality (16dimensional): quality指标
//! - Metadata (32dimensional): EXIF and fileproperty
//! - Context (16dimensional): 上下文information
...
fn extract_color_features(img: &DynamicImage) -> Vec<f64> {
 ...
 // 🚀 optimization1: intelligentsampling - 大图片sampling，小图片全量
 let step = if total_pixels > 1_000_000 { 20 } else if total_pixels > 100_000 { 10 } else { 1 };
 
 // 🚀 optimization2: single次遍历calculation所 has statistics量
 let mut r_sum = 0u64;
 let mut g_sum = 0u64;
 let mut b_sum = 0u64;
 ...
 // HSVconversion（every10pixelsamplinga次）
 ...
 // 🚀 optimization3: second次遍历calculation方差（仅atneed when ）
 let mut r_var = 0.0;
 let mut g_var = 0.0;
 let mut b_var = 0.0;
...
fn extract_shape_features(img: &DynamicImage, features: &crate::ImageFeatures) -> Vec<f64> {
 ...
 // boundary框紧密度（detectionnot矩形内容）
 let bbox_tightness = calculate_bbox_tightness(&rgba);
 
 // 内容分布（detection内容is否集at心）
...
fn extract_metadata_features(path: &Path) -> Vec<f64> {
 ...
 features[0] = 1.0; // file存
 features[1] = (metadata.len() as f64).ln(); // logfile大小
 
 // modifiedtime（Unixtime戳normalize）
 if let Ok(modified) = metadata.modified()
 && let Ok(duration) = modified.duration_since(std::time::UNIX_EPOCH) {
 features[2] = (duration.as_secs() as f64 / 1e9).ln(); // 归a化to合理range
 }
 }
 ...
 // note：this里use简implementation，fullEXIFneed额外dependency
 // features[5..15] 保留 to EXIFdata
...
/// 🚀 optimization版：估算唯acolorcount（直接from Image Buffersampling）
...
/// 估算colorcount（保留旧versioncompatibility性）
#[allow(dead_code)]
...
fn calculate_center_of_mass(rgba: &image::RgbaImage) -> (f64, f64) {
 ...
 for x in 0..width {
 let pixel = rgba.get_pixel(x, y);
 let weight = pixel[3] as f64 / 255.0; // usingalpha作for权重
...
fn calculate_blank_ratio(rgba: &image::RgbaImage) -> f64 {
 ...
 // detectionis否forempty白（白色ortransparency）
