...
/// Calculate SSIM（version）
///
/// note：thisisa SSIM implementation，for quick quality checking。
...
// samplingcalculation（every 8x8 blocksamplingatimes，highperformance）
 let sample_step = 8;
...
/// fullquality（contains SSIM、PSNR、MSE）
#[derive(Debug, Clone)]
pub struct AdvancedQualityMetrics {
/// structuresimilarity (0.0-1.0, morehighmore)
 pub ssim: f64,
/// Peak signal-to-noise ratio (d B, morehighmore)
 pub psnr: f64,
/// Mean squared error (morelowmore)
 pub mse: f64,
/// qualityevaluate
...
impl QualityChecker {
/// calculationfullquality（SSIM + PSNR + MSE）
...
// onlycalculation RGBchannel（ignorealpha）
 for i in 0..3 {
...
///
/// Inspired by Pio project design，usetwosearchautofindtoOptimal quality parameter
...
///
/// usetwosearchalgorithm，atspecifyqualityrangeinsidefindtomeettargetSSIMlowest qualityparameter
///
/// # parameter
...
// SSIM meets requirement，trymorelowquality
 best_quality = mid;
 best_ssim = ssim;
 best_size = file_size;
 high = mid - 1;
 } else {
// SSIM not ，needmorehighquality
 low = mid + 1;
 }
 }
