...
export function useRustCLI() {
      ...
      for (let i = 0; i < mediaFiles.length; i++) {
        ...
        if (options.format === null || options.isFilterMode) {
          ...
          if (!cleanFormat || cleanFormat.length === 0) {
            // 🔥 响亮报错：滤镜模式必须有原格式
            const error = new Error(`Filter mode requires original format, but file "${file.name}" has no extension`)
            logger.error(LOG_KEYS.RUST_CLI_ERROR, '❌ Filter mode failed: no original format', {
              file: file.name,
              ext: file.ext
            })
            throw error
          ...
          cleanFormat = options.format
          if (!cleanFormat) {
            // 🔥 响亮报错：非滤镜模式必须指定格式
            const error = new Error('Target format is required for non-filter mode conversion')
            logger.error(LOG_KEYS.RUST_CLI_ERROR, '❌ No target format specified')
            throw error
          }
          ...
          if (!cleanFormat || cleanFormat.length === 0) {
            // 🔥 响亮报错：格式清理后为空
            const error = new Error(`Invalid format after cleaning: "${options.format}"`)
            logger.error(LOG_KEYS.RUST_CLI_ERROR, '❌ Format became empty after cleaning', {
  ...
  const convert = async (options) => {
    ...
    if (options.disableFormatChange || options.format === null) {
      ...
      if (!originalExt) {
        // 🔥 响亮报错：滤镜模式必须有原格式
        const error = new Error(`Filter mode requires original format, but file has no extension: ${options.inputPath}`)
        logger.error(LOG_KEYS.RUST_CLI_ERROR, '❌ Filter mode failed: no original format', {
