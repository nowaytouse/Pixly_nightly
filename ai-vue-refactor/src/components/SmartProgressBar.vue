...
const updateVisualProgress = () => {
  if (!props.smartSmoothing) {
    visualProgress.value = props.progress;
    return;
  }
  ...
  if (props.status === 'processing' || Math.abs(target - visualProgress.value) > 0.1) {
...
onMounted(() => {
  visualProgress.value = props.progress;
  if (props.smartSmoothing && props.status === 'processing') {
    updateVisualProgress();
  }
});
