...
enum Commands {
    ...
    Video {
        ...
        /// Video codec (h264, h265, h266, av1, vp9, auto=keep original)
        /// 🔮 滤镜模式：使用 "auto" 保持原编码器
...
fn run(cli: Cli) -> Result<()> {
    match cli.command {
        ...
        Commands::Video {
            ...
            // 🔮 滤镜模式：当 codec = "auto" 时，使用输入视频的原始编码器
            let actual_codec = if final_codec == "auto" {
                // 分析输入视频获取原始编码器
                let processor = VideoProcessor::new();
                match processor.analyze_video(&input) {
                    Ok(info) => {
                        println!("🔮 Filter mode: Using original codec '{}'", info.codec);
                        info.codec
                    }
                    Err(e) => {
                        eprintln!("⚠️ Failed to detect original codec: {}", e);
                        eprintln!("   Falling back to h265");
                        "h265".to_string()
                    }
                }
            } else {
            ...
            let mut config = VideoConversionConfig {
                codec: actual_codec,  // Use detected or ML predicted codec
