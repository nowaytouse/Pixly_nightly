...
```javascript
// ✅ 真实HTTP调用
const response = await fetch('http://localhost:50052/api/v1/training/stats');
if (!response.ok) throw new Error('AI service not available');
```
...
### 2. 架构严格分离
- Go+Python: **仅**AI预测和机器学习，**禁止**转换和文件处理
- Rust: **仅**转换执行和文件处理，**禁止**参数决策
