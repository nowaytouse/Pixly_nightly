...
func (pv31 *PredictorV31) PredictWithCustomTarget(
	...
	// 提取特征
	features, err := pv31.PredictorImpl.featureExtractor.ExtractFeatures(filePath)
	if err != nil {
...
func (pv31 *PredictorV31) PredictOptimalParamsWithTuning(filePath string) (*Prediction, error) {
	// 先使用v3.0黄金规则预测
	prediction, err := pv31.PredictorImpl.PredictOptimalParams(filePath)
	if err != nil {
		return nil, err
	}
