...
// HasSeedData 检查知识库中是否已有种子数据
func (d *Database) HasSeedData() bool {
	var count int
	row := d.db.QueryRow("SELECT COUNT(*) FROM conversion_records WHERE prediction_rule LIKE 'seed-%' OR prediction_rule LIKE 'real-%'")
...
// SeedDefaultRecords 导入预设的种子记录
func (d *Database) SeedDefaultRecords(records []*ConversionRecord) error {
	d.logger.Info("🌱 开始导入种子数据",
		zap.Int("count", len(records)))

	successCount := 0
	for _, record := range records {
		if err := d.SaveRecord(record); err != nil {
			d.logger.Warn("导入种子记录失败",
